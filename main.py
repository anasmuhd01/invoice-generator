...
# filename,date these two variables will contain the list value as filename - [0], date - [1]
    ...
    pdf.cell(w=70, h=8, txt="Price", border=1)
    pdf.cell(w=70, h=8, txt="Total_price", border=1, ln=1)
    for index, row in df.iterrows():
        ...
        pdf.cell(w=70, h=8, txt=f"{row['price_per_unit']}", border=1)
        pdf.cell(w=70, h=8, txt=f"{row['total_price']}",border=1, ln=1)
    pdf.set_font(family="Times", size=8)
